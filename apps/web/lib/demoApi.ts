...
export function demoRequestJson(path: string, method: "GET" | "POST" | "PUT" | "DELETE", _body?: object): unknown {
  const requestUrl = new URL(path, "https://demo.ma-core.local");
  if (requestUrl.pathname === "/api/auth/me") return sessionPayload();
  if (requestUrl.pathname === "/api/auth/register") return sessionPayload();
  if (requestUrl.pathname === "/api/auth/login") return sessionPayload();
  if (requestUrl.pathname === "/api/auth/refresh") return sessionPayload();
  if (requestUrl.pathname === "/api/auth/logout") return { ok: true };
  if (requestUrl.pathname === "/api/events/recent") return recentEvents();
  if (requestUrl.pathname === "/api/signals") return signals();
  if (requestUrl.pathname === "/api/strategies") return strategies();
  if (requestUrl.pathname === "/api/market/adapters/status") return adapterStatuses();
  if (requestUrl.pathname === "/api/risk/state") return riskStateResponse();
  if (requestUrl.pathname === "/api/risk/events") return riskEvents();
  if (requestUrl.pathname === "/api/risk/policy") return riskPolicy();
  if (requestUrl.pathname === "/api/execution/kill-switch") return { lock: { id: "lock_manual_2026", userId: operatorId, accountId: "22222222-2222-4222-8222-222222222222", lockType: "GLOBAL_TRADING_LOCK", reason: "MANUAL_LOCK", active: true, lockUntil: new Date(now.getTime() + 3600000).toISOString(), createdAt: now.toISOString() }, closeResult: { closed: 1, failed: 0 } };
  if (requestUrl.pathname === "/api/execution/mode") return { ok: true };
  if (requestUrl.pathname.startsWith("/api/execution/positions/") && requestUrl.pathname.endsWith("/close")) return { position: { ...firstPosition(), status: "FORCE_CLOSE_REQUESTED", forceCloseRequestedAt: now.toISOString() } };
  if (requestUrl.pathname.startsWith("/api/execution/positions/") && requestUrl.pathname.endsWith("/sync")) return { position: firstPosition(), syncQueued: true };
  if (requestUrl.pathname === "/api/positions/open") return positions();
  if (requestUrl.pathname === "/api/executions") return executions();
  if (requestUrl.pathname.startsWith("/api/execution/decisions/")) return executions()[0];
  if (requestUrl.pathname === "/api/execution/orders") return [];
  if (requestUrl.pathname === "/api/execution/audit") return [];
  if (requestUrl.pathname === "/api/private-streams") return privateStreams();
  if (requestUrl.pathname === "/api/live-readiness") return liveReadiness();
  if (requestUrl.pathname === "/api/live-readiness/checks") return liveReadiness()[0] ?? { ok: true };
  if (requestUrl.pathname === "/api/reconciliation/runs") return reconciliationRuns();
  if (requestUrl.pathname === "/api/reconciliation/mismatches") return reconciliationMismatches();
  if (requestUrl.pathname === "/api/incidents") return incidents();
  if (requestUrl.pathname === "/api/safe-mode") return method === "POST" ? safeModeEvents()[0] : safeModeEvents();
  if (requestUrl.pathname.startsWith("/api/safe-mode/") && requestUrl.pathname.endsWith("/resolve")) return { ...safeModeEvents()[0], active: false, resolvedAt: now.toISOString() };
  if (requestUrl.pathname === "/api/operations/command-center") return operationsHealth();
  if (requestUrl.pathname === "/api/portfolio/protection") return portfolioSnapshot();
  if (requestUrl.pathname === "/api/forensic-audit") return method === "POST" ? forensicCases()[0] : forensicCases();
  if (requestUrl.pathname === "/api/approval-requests") return method === "POST" ? approvalRequests()[0] : approvalRequests();
  if (requestUrl.pathname.startsWith("/api/approval-requests/") && requestUrl.pathname.endsWith("/decision")) return { ...approvalRequests()[0], status: "APPROVED", decidedAt: now.toISOString() };
  if (requestUrl.pathname === "/api/disaster-recovery") return disasterRecoveryRuns();
  if (requestUrl.pathname === "/api/disaster-recovery/run") return disasterRecoveryRuns()[0];
  if (requestUrl.pathname === "/api/compliance/status") return complianceAcceptances();
  if (requestUrl.pathname === "/api/compliance/accept") return complianceAcceptances()[0];
  if (requestUrl.pathname === "/api/test-evidence") return method === "POST" ? testEvidenceReports()[0] : testEvidenceReports();
  if (requestUrl.pathname === "/api/live-readiness/wizard") return liveReadinessWizard();
  if (requestUrl.pathname === "/api/live-readiness/wizard/step") return liveReadinessWizard();
  if (requestUrl.pathname === "/api/ops/outbox") return outboxItems();
  if (requestUrl.pathname === "/api/ops/metrics") return metrics();
  if (requestUrl.pathname === "/api/ops/streams/metrics") return streamMetrics();
  if (requestUrl.pathname === "/api/ops/streams/dead-letter") return [];
  if (requestUrl.pathname === "/api/agents/market-analysis/run") return { ok: true, transaction_id: "tx_mock_market_cycle_2026" };
  if (requestUrl.pathname === "/api/exchanges/connect") return { message: "Demo credentials were validated locally and encrypted in a mock vault state." };
  return { message: `Demo API route is not registered: ${requestUrl.pathname}` };
}

export function demoLiveEnvelope(index: number): AgentEnvelope {
  const sequence = [marketVectorEnvelope(), executionStatusEnvelope(), riskStateEnvelope(), positionTimeoutEnvelope(), strategyFeedEnvelope(), securityAuditEnvelope()];
  const base = sequence[index % sequence.length] ?? marketVectorEnvelope();
  return { ...base, timestamp: new Date(Date.now()).toISOString(), idempotency_key: `${base.idempotency_key}_${index}_${Date.now()}` };
}

export function demoLiveEnvelopes(): readonly AgentEnvelope[] {
  return [marketVectorEnvelope(), executionStatusEnvelope(), riskStateEnvelope(), positionTimeoutEnvelope(), strategyFeedEnvelope(), securityAuditEnvelope()];
}
